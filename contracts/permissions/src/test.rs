...
mod test {
    ...
    fn test_merchant_in_whitelist_succeeds() {
        ...
        assert!(client.grant(&owner, &delegate, &1000, &100, &merchants, &10000).is_ok());
    ...
    #[test]
    fn test_grant_with_zero_ttl_fails() {
        let env = Env::default();
        let owner = Address::generate(&env);
        let delegate = Address::generate(&env);
        ...
        let merchants = Vec::new(&env);
        let result = client.try_grant(&owner, &delegate, &1000, &100, &merchants, &0);
        
        assert_eq!(result, Err(Ok(crate::PermissionError::ExpiryInPast)));
    }

    #[test]
    fn test_grant_with_one_ledger_succeeds() {
        let env = Env::default();
        let owner = Address::generate(&env);
        let delegate = Address::generate(&env);
        ...
        let merchants = Vec::new(&env);
        let result = client.grant(&owner, &delegate, &1000, &100, &merchants, &1);
